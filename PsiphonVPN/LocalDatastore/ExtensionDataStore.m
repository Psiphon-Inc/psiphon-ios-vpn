...
#pragma mark - Subscription metadata

- (NSString*)lastAuthID {
    return [self.dataStore lookup:LastAuthIDKey];
}

- (void)setLastAuthID:(NSString*)lastAuthID {

    [self.dataStore insert:lastAuthID key:LastAuthIDKey];
}

- (NSString*)lastAuthAccessType{
    return [self.dataStore lookup:LastAuthAccessTypeKey];
}

- (void)setLastAuthAccessType:(NSString*)lastAuthAccessType {
    [self.dataStore insert:lastAuthAccessType key:LastAuthAccessTypeKey];
}
