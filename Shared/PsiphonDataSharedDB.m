...
- (BOOL)createDatabase {
    ...
    [q inDatabase:^(FMDatabase *db) {
        ...
        if (!success) {
            ERROR(@"createDatabase: %@", [db lastError]);
        }
    }];
...
- (BOOL)clearDatabase {
    ...
    [q inDatabase:^(FMDatabase *db) {
        ...
        if (!success) {
            ERROR(@"clearDatabase error: %@", [db lastError]);
        }
    }];
...
- (BOOL)updateHomepages:(NSArray<NSString *> *)homepageUrls {
    ...
    [q inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        if (!success) {
            ERROR(@"updateHomepages: rolling back, error %@", [db lastError]);
            *rollback = TRUE;
            return;
        }
    }];
...
- (NSArray<Homepage *> *)getAllHomepages {
    ...
    [q inDatabase:^(FMDatabase *db) {
        ...
        if (rs == nil) {
            ERROR(@"getAllHomepages: error %@", [db lastError]);
            return;
        }
...
- (BOOL)insertNewEgressRegions:(NSArray<NSString *> *)regions {
    ...
    [q inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        if (!success) {
            ERROR(@"insertNewEgressRegions: rolling back, error %@", [db lastError]);
            *rollback = TRUE;
            return;
        }
    }];
...
- (NSArray<NSString *> *)getAllEgressRegions {
    ...
    [q inDatabase:^(FMDatabase *db) {
        ...
        if (rs == nil) {
            ERROR(@"getAllEgressRegions: error %@", [db lastError]);
            return;
        }
...
- (BOOL)insertDiagnosticMessage:(NSString*)message {
    ...
    [q inDatabase:^(FMDatabase *db) {
        ...
        if (!success) {
            ERROR(@"insertDiagnosticMessage: error %@", err);
...
- (NSArray<DiagnosticEntry*>*)getLogsNewerThanId:(int)lastId {
    ...
    [q inDatabase:^(FMDatabase *db) {
        ...
        if (rs == nil) {
            ERROR(@"getLogsNewerThanId: error %@", [db lastError]);
            return;
        }
...
- (BOOL)updateTunnelConnectedState:(BOOL)connected {
    ...
    [q inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        if (!success) {
            ERROR(@"updateTunnelConnectedState: rolling back, error %@", [db lastError]);
            *rollback = YES;
            return;
        }
    }];
...
- (BOOL)getTunnelConnectedState {
    ...
    [q inDatabase:^(FMDatabase *db) {
        ...
        if (rs == nil) {
            ERROR(@"getTunnelConnectedState: error %@", [db lastError]);
            return;
        }

        if (![rs next]) {
            ERROR(@"getTunnelConnectedState: No previous data recorded.");
            return;
        }
...
- (BOOL)updateAppForegroundState:(BOOL)foreground {
    ...
    [q inTransaction:^(FMDatabase *db, BOOL *rollback) {
        ...
        if (!success) {
            ERROR(@"updateAppForegroundState: rolling back, error %@", [db lastError]);
            *rollback = YES;
            return;
        }
    }];
...
- (BOOL)getAppForegroundState {
    ...
    [q inDatabase:^(FMDatabase *db) {
        ...
        if (rs == nil) {
            ERROR(@"getAppForegroundState: error %@", [db lastError]);
            return;
        }

        if (![rs next]) {
            ERROR(@"getAppForegroundState: failed to retrieve row successfully. Aborting.");
            abort();
        }
